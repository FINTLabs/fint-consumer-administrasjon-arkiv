...
public class RolleController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Rolle cache is disabled.");
        }
    ...
    @GetMapping
    public RolleResources getRolle(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ArkivActions.GET_ALL_ROLLE, client);
        event.setOperation(Operation.READ);
        fintAuditService.audit(event);
        fintAuditService.audit(event, Status.CACHE);

        List<RolleResource> rolle;
        ...
        return linker.toResources(rolle);
    }


    @GetMapping("/systemid/{id:.+}")
    public RolleResource getRolleBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ArkivActions.GET_ROLLE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<RolleResource> rolle = cacheService.getRolleBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return rolle.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            RolleResource rolle = objectMapper.convertValue(response.getData().get(0), RolleResource.class);
