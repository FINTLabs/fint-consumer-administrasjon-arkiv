...
public class KlasseController {
    ...
    public KlasseResources getKlasse(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ArkivActions.GET_ALL_KLASSE, client);
    ...
    @GetMapping("/klasseid/{id:.+}")
    public KlasseResource getKlasseByKlasseId(
        ...
        log.debug("klasseId: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, ArkivActions.GET_KLASSE, client);
        event.setOperation(Operation.READ);
        event.setQuery("klasseId/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<KlasseResource> klasse = cacheService.getKlasseByKlasseId(orgId, id);
